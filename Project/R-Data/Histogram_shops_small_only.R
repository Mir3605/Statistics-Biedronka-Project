...
shops_number <- dbGetQuery(mydb,'SELECT liczba_sklepow FROM liczba_sklepow WHERE liczba_sklepow < 7 AND liczba_sklepow > 0')
dbDisconnect(mydb)

png(file='Histogram_shops_small_only.png', width=900, height = 800)

par(mar = c(6,6,6,2))

hist(shops_number$liczba_sklepow, main="Histogram liczby sklepów\ndo sześciu sklepów", xlab="Liczba sklepów",
