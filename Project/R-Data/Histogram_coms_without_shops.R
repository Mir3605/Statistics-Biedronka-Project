...
citizens_number <- dbGetQuery(mydb,'SELECT liczba_mieszkancow FROM gminy WHERE liczba_mieszkancow <=50000')
dbDisconnect(mydb)

png(file='Histogram_coms_without_shops.png', width=900, height = 800)

par(mar = c(6,6,6,2))

hist(citizens_number$liczba_mieszkancow, main="Histogram liczby mieszkańców wszystkich\ngmin poniżej 50 tys. mieszkańców", 
